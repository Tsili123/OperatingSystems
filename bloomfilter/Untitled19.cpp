...
static int check_recursive(RBNODE_T *node, bool must_be_black, 
    int (*cmp)(const char*, const char*), int *black_nodes_to_leaf) {
...
RBTREE_T *rbtree_create(
    int (*key_compare) (const char*, const char*), 
    char* (*key_copy)(const char*, const char*), 
    void (*key_free) (void*), 
 ...
 */
void rbtree_alter_key_copy(RBTREE_T *tree, char* (*key_copy)(const char*,const char*)) {
  strcpy(tree->key_copy , key_copy);
}
...
RBNODE_T* rbtree_lookup(RBTREE_T *tree, void *key, bool create, bool *created) {
  ...
  if (create) {
    ...
    if (tree->key_copy != NULL) {
      next->key = tree->key_copy(next->key,key);
    } else {
      next->key = key;
    }
