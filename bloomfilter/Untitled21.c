...
typedef struct PCNode *PCNodePtr;
...
struct PCNode {	
	PCNodePtr PrevPC, NextPC;
	RBNodePtr FirstRBNode;
	RBNodePtr UpRBNode;
	int data;
};
//I added some tabnode pointers for the bonus function
struct RBNode{
	RBNodePtr PrevRBNode, NextRBNode;
	PCNodePtr UpPCNode;
	RBNODE_T *pointer;//pointer to red black tree
};
...
InfoPtr InfoNew()
{
	...
	PC = malloc(sizeof(struct PCNode));
...
//	printf("%p\n",Session->CurrTab->PrevTab->PrevTab);
	if(Session->CurrTab->PrevTab==Session->CurrTab)
	{
