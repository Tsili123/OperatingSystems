...
int sup(RBNODE_T *root,bool voters)
{
	int counter=1;
	//base case
    if (root == NULL)
    ...
    counter+=sup(root->right,voters);
    
    return counter;
}

bool search(RBTREE_T *t,RBNODE_T *root,char *key)
{
	if(root==NULL){
		return false;
	}
	
	if(t->key_compare(key,root->key)==0){
		return true;
	}
	else if(t->key_compare(key,root->key)<0){
		search(t,root->left, key);
	}
	else if(t->key_compare(t,root->key)>0){
		search(t,root->right, key);
	}
}

RBNODE_T* search_n(RBTREE_T *t,RBNODE_T *root,char *key,bool vote)
{
	...
	if(t->key_compare(key,root->key)==0){
		...
		search_n(t,root->left, key,vote);
	}
	else if(t->key_compare(t,root->key)>0){
		search_n(t,root->right, key,vote);
	}
}



bool check_node_vote(RBNODE_T *node)
{
...
static int check_recursive(RBNODE_T *node, bool must_be_black, 
  ...
  if (node->is_red && must_be_black) 
  {
		printf("A node that must be black is red\n");
		return 0;
  }
  ...
  if (node->left != NULL) {
	...
	{
		printf("Left node has wrong parent node\n"); 
		return 0;
	}
    if (cmp(node->key, node->left->key) < 0) 
	{
		printf("Left node has larger key\n"); 
		return 0;
	}
  ...
  if (node->right != NULL) {
	...
	{
		printf("Right node has wrong parent node\n"); 
		return 0;
	}
    if (cmp(node->key, node->right->key) > 0) 
	{
		printf("Right node has smaller key\n");
		return 0;
	}
    count += check_recursive(node->right, node->is_red, cmp, &right_black_nodes);
  }
  if (left_black_nodes != right_black_nodes)
    printf("Number of black nodes in a simple path to a left leaf node must be the same as the right leaf node\n");
...
void rbtree_check(RBTREE_T *tree) {
  ...
  if (tree->key_compare == NULL) 
  {
  		printf("key_compare is null\n");
		  return;	
  }
  if (tree->size == 0) {
    if (tree->root != NULL) 
	{
		printf("Root expected to be null as tree is empty\n");
		return;	
	}
  } else {
    ...
    if (count != tree->size)
    {
    	printf("Mismatch between recorded size and actual node count\n");	
	}
  }
}
 ...
 */         
void rbtree_set(RBTREE_T *tree, RBNODE_T *node, void *value , void *name , void *surname , int *age ,char *gender,int *pc) {
  ...
  //if (tree->key_compare(node->name ,name)!=0) {
    //if (node->name) {
     // if (tree->key_free) tree->key_free(node->name); 
   // }
    if (tree->key_copy) {
      node->name = tree->key_copy(name);
    } 
  //}
  
    if (tree->key_copy) {
      node->surname = tree->key_copy(surname);
    } 
  
  
    if (node->age != age) {
    if (node->age) {
      if (tree->value_free) tree->value_free(node->pc); 
    }
    if (tree->value_copy) {
      node->age = tree->value_copy(age);
    } else {
      node->age = age;
    }
  }
  ...
  if (node->gender != gender) {
    ...
    if (tree->value_copy) {
      node->gender = tree->value_copy(gender);
    } else {
      node->gender = gender;
    }
  }
 ...
 */
RBNODE_T* rbtree_put(RBTREE_T *tree, void *key, void *value, void *name , void *surname , int *age ,char *gender,int *pc ) {
  RBNODE_T *node;
  bool created;
  node = rbtree_lookup(tree, key, true, &created);
  rbtree_set(tree, node, value ,name,surname,age,gender,pc);
  if (!created) {
 ...
 */
bool rbtree_make(RBTREE_T *tree, void *key, void *value, void *name , void *surname , int *age ,char *gender,int *pc ) {
  RBNODE_T *node;
  bool created;
  node = rbtree_lookup(tree, key, true, &created);
  if (created) {
