...
int semaphore_create(void){
    ...
    semaphore_id++; // Increment this, so the next semaphore we create has a totaly new id */
...
int semaphore_getvalue(int sem_id){
    /* Returns value of semaphore , returns -1 on ERROR, */
	...
	semValue = semctl(sem_id, 0, GETVAL, arg);
